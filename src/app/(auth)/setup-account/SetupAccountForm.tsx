...
export function SetupAccountForm() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    e.preventDefault();
    if (!token) return toast.error('Missing setup token');
    
    const errs = validatePasswordPolicy(password, {});
    ...
    try {
      ...
      if (!res.ok) throw new Error(data.error?.message || 'Failed to setup account');
